...
const verComentarios=async(idpost)=>{
    ...
    let datos=new FormData();
    datos.append("idpost",idpost);
    datos.append("action","cargarComentarios");
    ...
    json.map(comen=>{
    var fecha3=moment(coment.fecha,'YYYY-MM-DD hh:mm:ss').fromNow();
    
    listaComentarios+=`
    <li class="list-group-item">
      <p><img src="${comen.foto}" width="20px" height="20px" class="rounded mx-2">
      <b class="mx-2">${comen.nombre}</b>  <small>${fecha3}</small></p>
      <p><b>${comen.comentario}</b></p>
    </li>
    `
    });
    listaComentarios+=`</ul>`
    document.getElementById(`collapse${p.idpost}`).innerHTML=listaComentarios;
    }
